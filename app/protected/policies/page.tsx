    ...
    (policy) =>
      ...
      (selectedLevel === null ||
        (policy.level_of_government &&
          policy.level_of_government.includes(selectedLevel))) // Check for null before includes
  );
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        {filteredPolicies.map((policy) => (
          <div
            key={policy.id}
            className="slide-up bg-card rounded-lg shadow-md dark:shadow-sm dark:shadow-gray-700 overflow-hidden cursor-pointer hover:scale-[1.02] transition-transform duration-300 ease-in-out p-5"
            onClick={() =>
              ...
              <div className="mt-auto">
                ...
                <div className="flex flex-wrap gap-2 mt-2">
                  {policy.tags.map(
                    (
                      tag,
                      index // Use tags instead of topics
                    ) => (
                      <span
                        key={tag + index}
